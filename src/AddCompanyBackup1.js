...
function DynamicForm() {
    ...
    // New state for "Same as Shipping" checkboxes
    ...
    const copyShippingToBilling = (billingEntryIndex, shippingEntryIndex = 0) => {
        const shippingCategory = categories.find(cat => cat.category_key === 'shippingaddress');
        const billingCategory = categories.find(cat => cat.category_key === 'billingaddress');
        
        if (!shippingCategory || !billingCategory) {
            console.warn('Shipping or Billing category not found');
            return;
        }

        console.log('Copying from shipping to billing...');
        console.log('Shipping category:', shippingCategory);
        console.log('Billing category:', billingCategory);

        setFormData(prevFormData => {
            const newFormData = { ...prevFormData };
            
            // Define field mapping between shipping and billing
            const fieldMapping = {
                // Shipping field -> Billing field
                'contact person name': 'billing_contact_person_name',
                'contact person email': 'billing_contact_person_email',
                'contact person mobile': 'billing_contact_person_mobile',
                'contact person designation': 'billing contact person designation',
                'store location': 'billing store location',
            ...
            Object.entries(fieldMapping).forEach(([shippingField, billingField]) => {
                const shippingFieldKey = `${shippingField}_${shippingEntryIndex}`;
                const billingFieldKey = `${billingField}_${billingEntryIndex}`;
                
                if (newFormData.hasOwnProperty(shippingFieldKey)) {
                    newFormData[billingFieldKey] = newFormData[shippingFieldKey];
                    console.log(`Copied ${shippingFieldKey}: ${newFormData[shippingFieldKey]} -> ${billingFieldKey}`);
                }
            });
    ...
    // Handle "Same as Shipping Address" checkbox change
    const handleSameAsShippingChange = (billingEntryIndex, checked) => {
        setSameAsShipping(prev => ({
            ...prev,
            [`billing_${billingEntryIndex}`]: checked
        }));

        if (checked) {
            // Get the first shipping address entry (index 0)
            const shippingEntryIndex = 0;
            copyShippingToBilling(billingEntryIndex, shippingEntryIndex);
        } else {
            ...
            if (billingCategory) {
                setFormData(prevFormData => {
                    ...
                    const billingFields = [
                        ...
                        'billing contact person mobile',
                        'billing contact person designation',
                        'billing store location',
                        'billing_country',
                        'billing_state',
                        'billing_city',
                        'billing address',
                    ...
                    billingFields.forEach(field => {
                        const fieldKey = `${field}_${billingEntryIndex}`;
    ...
    const removeCategoryEntry = (categoryKey, entryIndex) => {
        ...
        setCategoryEntries(prev => {
            ...
            // Remove same as shipping state for this entry
            setSameAsShipping(prev => {
                const newState = { ...prev };
                delete newState[`billing_${entryIndex}`];
                return newState;
            });
    ...
    const handleReset = () => {
        ...
        setSameAsShipping({}); // Reset same as shipping checkboxes
    };

    // Modified handleSubmit function - only the submission part
    const handleSubmit = async (e) => {
        ...
        if (isValid) {
            // Group data by categories and entries
            const groupedData = {};
            ...
            // Prepare the submission data with grouped structure
            const submissionData = {
                type: "insert",
            ...
            axios.post('https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/AddCustomer.php', submissionData)
                .then(res => {
    ...
    // Render "Same as Shipping Address" checkbox for billing address sections
    const renderSameAsShippingCheckbox = (categoryKey, entryIndex) => {
        if (categoryKey !== 'billingaddress') return null;

        const checkboxKey = `billing_${entryIndex}`;
        const isChecked = sameAsShipping[checkboxKey] || false;

        // Check if shipping address has any data
        const hasShippingData = categories.find(cat => cat.category_key === 'shippingaddress')?.fields?.some(field => {
            const shippingFieldKey = `${field.field_name}_0`;
            return formData[shippingFieldKey] && formData[shippingFieldKey].toString().trim() !== '';
        });
        ...
        return (
            <div className="mb-4 p-3 rounded border" style={{
                ...
                <Form.Check
                    type="checkbox"
                    id={`same-as-shipping-${entryIndex}`}
                    label="Same as Shipping Address"
                    checked={isChecked}
                    onChange={(e) => handleSameAsShippingChange(entryIndex, e.target.checked)}
                    className={`fw-bold ${theme === 'dark' ? 'text-light' : 'text-primary'}`}
                    style={{
                        fontSize: '1.1rem'
                    }}
                />
                <Form.Text className={`mt-1 ${theme === 'dark' ? 'text-light opacity-75' : 'text-muted'}`}>
                    <small>
                        <i className="bi bi-info-circle me-1"></i>
                        Check this box to copy all shipping address details to this billing address
                    </small>
                </Form.Text>
            </div>
        );
                                                    ...
                                                    >
                                                        {(categoryEntries[category.category_key] || [0]).map((entryIndex, idx) => (
                                                            <div key={entryIndex} className="mb-4">
                                                                ...
                                                                {/* Render "Same as Shipping Address" checkbox for billing address */}
                                                                {renderSameAsShippingCheckbox(category.category_key, entryIndex)}
