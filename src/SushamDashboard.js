...
const SushamDashboard = () => {
    ...
    const formTitle = "Susham Dashboard";

    // Fetch data from API
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch("https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/susham/SushamDashboardApi.php");
    ...
    const SaveButtonRenderer = (params) => {
        const handleSave = async () => {
            try {
                ...
                // Format dates for API (convert to YYYY-MM-DD if needed)
                ...
                const payload = {
                    file: params.data.FILE_ID || params.data.file_name || '',
                    material_received_status: params.data.material_received_status || '',
                    startDate: formatDateForAPI(params.data.startDate),
                    shm: params.data.shm || '0',
                    finish: params.data.finish || '0',
                    totalSq: params.data.total_sqft || '0',
                    finish1: params.data.finish1 || '0',
                ...
                // Call save API
                const response = await fetch('https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/susham/saveSushamDashboardApi.php', {
                    method: 'POST',
                    headers: {
    ...
    const columnDefs = [
        ...
        },
        {
            ...
            cellRenderer: DropdownRenderer,
            cellEditorParams: {
                values: ["Select Remark", "RFD", "Dispatch"]
            },
            cellStyle: { backgroundColor: '#fff', padding: '0' }
        },
        {
    ...
    const downloadExcel = () => {
        ...
        try {
            const params = {
                fileName: `Susham_Dashboard_${new Date().toISOString().split('T')[0]}.csv`,
                allColumns: true
            };
...
export default SushamDashboard;