...
function DynamicForm() {
    ...
    const [fileTypeEntries, setFileTypeEntries] = useState({}); // Track entries per file type
    ...
    const handleChange = (e, change_rule_id, entryIndex = 0) => {
        ...
        if (name.toLowerCase().includes('file_type')) {
            const entryKey = `${entryIndex}`; // Create unique key for each entry
            setSelectedFileTypes(prev => ({
                ...prev,
                    ...
                    if (Array.isArray(allRuleResults)) {
                        ...
                        allRuleResults.forEach(ruleResult => {
                            ...
                            if (formulla) {
                                ...
                                // Update categories with fields for this specific entry
                                const updatedCategories = futureCategories.map(category => {
                                    if (category.id == '11') {
                                        const filteredFields = category.fields
                                            .filter(field => {
                                                return field.field_name.toLowerCase().includes('file_type') ||
                                                    (field.group && field.group.toLowerCase() === currentEntryFileType);
                                            })
                                            .map(field => {
                                ...
                                // Initialize table rows for AMC type
    ...
    const getGroupedFields = (categoryId) => {
        ...
        futureCategories.forEach(category => {
            if (category.id == categoryId) {
                category.fields.forEach(field => {
                    if (field.group && field.group.toLowerCase() !== '' && !field.field_name.toLowerCase().includes('file_type')) {
    ...
    const getNonGroupedFields = (categoryFields, selectedFileType, entryIndex = 0) => {
        return categoryFields.filter(field => {
            ...
            // For other fields, check if they should be shown based on selected file type FOR THIS ENTRY
            if (field.group && field.group.toLowerCase() !== '') {
                if (selectedFileType && field.group.toLowerCase() === selectedFileType.toLowerCase()) {
                    return selectedFileType.toLowerCase() !== 'amc';
                }
                return false;
            }
    ...
    const getHighlightedFieldsForForm = (categoryId, selectedFileType, entryIndex = 0) => {
        if (categoryId != 11 || selectedFileType.toLowerCase() !== 'amc') return [];
                                        ...
                                        {categories.map((category, categoryIndex) => (
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    {(categoryEntries[category.category_key] || [0]).map((entryIndex, idx) => {
                                                        ...
                                                        return (
                                                            <div key={entryIndex} className="mb-4">
                                                                    ...
                                                                    <Row>
                                                                        {category.id == 11 && (() => {
                                                                            if (currentEntryFileType.toLowerCase() === 'amc') {
                                                                                ...
                                                                                // Show all grouped fields as regular form fields for non-AMC types
                                                                ...
                                                                {/* Show tables for AMC type per entry */}
                                                    ...
                                                    {/* Add entry button for multiple categories */}
                                                    {category.is_multiple === 'yes' && (
