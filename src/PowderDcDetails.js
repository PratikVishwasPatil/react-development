...
const ConsolidatedDCReportPowder = () => {
    ...
    // API URLs
    const REPORT_API_URL = "http://erp.suryaequipments.com/Surya_React/surya_dynamic_api/Store/get_consolidateddcreportPowderApi.php";
    ...
    const fetchReportData = async (financialYear) => {
        ...
        try {
            ...
            if (data.status && data.data) {
                ...
                showToast(`Loaded ${data.total || data.data.length} powder DC records for FY ${financialYear}`, 'success');
            } else {
    ...
    const columnDefs = useMemo(() => [
        ...
        {
            headerName: "Material/Powder Name",
            field: "materialname",
            filter: "agTextColumnFilter",
            sortable: true,
            width: 350,
            cellStyle: params => {
                // Color code based on powder type
                if (params.value?.includes('POWDER-OXFORD BLUE')) {
                    return { backgroundColor: theme === 'dark' ? '#1e3a5f' : '#e3f2fd', color: theme === 'dark' ? '#90caf9' : '#1565c0', fontWeight: '500' };
                } else if (params.value?.includes('POWDER-BRIGHT ORANGE')) {
                    return { backgroundColor: theme === 'dark' ? '#5f2e1e' : '#fff3e0', color: theme === 'dark' ? '#ffb74d' : '#e65100', fontWeight: '500' };
                } else if (params.value?.includes('POWDER-BS GREY')) {
                    return { backgroundColor: theme === 'dark' ? '#2e2e2e' : '#f5f5f5', color: theme === 'dark' ? '#bdbdbd' : '#424242', fontWeight: '500' };
                } else if (params.value?.includes('POWDER-NEW HAVELLS GREY')) {
                    return { backgroundColor: theme === 'dark' ? '#2e3a2e' : '#e8f5e9', color: theme === 'dark' ? '#81c784' : '#2e7d32', fontWeight: '500' };
                } else if (params.value?.includes('POWDER-BLUE-VIP-KRIPTON')) {
                    return { backgroundColor: theme === 'dark' ? '#1e2f5f' : '#e8eaf6', color: theme === 'dark' ? '#9fa8da' : '#3f51b5', fontWeight: '500' };
                }
    ...
    const onGridReady = (params) => {
        console.log('Consolidated Powder DC Report Grid is ready');
    ...
    const handleExportCSV = () => {
        if (gridRef.current && gridRef.current.api) {
            gridRef.current.api.exportDataAsCsv({
                fileName: `Powder_DC_Report_${selectedYear}_${new Date().toISOString().split('T')[0]}.csv`
            });
    ...
    const totalWeight = useMemo(() => {
        return reportData.reduce((sum, row) => sum + (parseFloat(row.weight) || 0), 0);
    }, [reportData]);
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'right', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem' }}>Loading powder DC report data...</p>
                </div>
            </div>
        );
    }
                        ...
                        <div style={{ 
                            ...
                            <div style={{ flex: '1 1 300px' }}>
                                <h4 style={{ margin: 0, marginBottom: '0.25rem' }}>
                                    🎨 Consolidated Powder DC Report
                                </h4>
                                <small style={{ opacity: 0.8 }}>
                                    {`${reportData.length} powder DC records | Total: ${totalWeight.toFixed(2)} kg`}
                    ...
                    <div style={{
                        ...
                        {reportData.length === 0 ? (
                            <div style={{
                                ...
                                <h5>No powder DC data available</h5>
...
export default ConsolidatedDCReportPowder;