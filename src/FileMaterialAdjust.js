...
const FileMaterialAdjustList = () => {
    ...
    // API URLs
    const FILE_API_URL = "https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/PPC/FileMaterialAdjustListApi.php";
    ...
    // Fetch file data
    const fetchFileData = async (financialYear) => {
        setLoading(true);
        try {
            const response = await fetch(`${FILE_API_URL}?FY=${financialYear}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
    ...
    const onSelectionChanged = (event) => {
        ...
        if (autoNavigateOnSelect && selectedData.length === 1) {
            ...
            setTimeout(() => {
                // Navigate to the file material detail page with FILE_ID
                navigate(`/ppc/file-material-adjust/${selectedRecord.FILE_ID}`);
            }, 500);
        }
    };
    ...
    const columnDefs = useMemo(() => [
        ...
        },
        {
            ...
            cellStyle: params => {
                ...
                } else if (params.value === "Purchase Order Not Send") {
    ...
    const onGridReady = (params) => {
        console.log('File Material Adjust List Grid is ready');
    ...
    const handleExportCSV = () => {
        if (gridRef.current && gridRef.current.api) {
            gridRef.current.api.exportDataAsCsv({
                fileName: `File_Material_Adjust_List_${selectedYear}_${new Date().toISOString().split('T')[0]}.csv`
            });
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p className="mt-3">Loading file data...</p>
                </div>
            </div>
        );
    }
                    ...
                    <Card.Header style={{
                        ...
                        <Row className="align-items-center">
                            <Col xs={12} lg={6} className="mb-2 mb-lg-0">
                                <h4 className="mb-0">File Material Adjust List</h4>
                                <small style={{ opacity: 0.8 }}>
                                    {`${fileData.length} records found for FY ${selectedYear}`}
                    ...
                    <Card.Body style={{
                        ...
                        {fileData.length === 0 ? (
                            <div style={{
                                ...
                                <h5>No file material data available</h5>
