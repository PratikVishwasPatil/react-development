...
const FATMaterialSelector = () => {
    ...
    {packingListData.map((row, index) => (
        <tr key={index} style={{
            ...
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.width || row.W_MM || row.wMm)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.height || row.H_L_MM || row.hLMm)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.quantity || row.QTY || row.qty)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.weight || row.WT || row.wt)}
            </td>
    ...
    {assignedPackingData.map((row, index) => (
        <tr key={index}>
            ...
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.width || row.w)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.height || row.hl || row.hLMm)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.quantity || row.design_qty || row.qty)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.weight || row.WT || row.wt)}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'right' }}>
                {formatNumber(row.fatQuantity || row.FAT_QUANTITY || row.fat_quantity || row.fat_qty || '1')}
            </td>
            <td style={{ padding: '6px', border: `1px solid ${themeStyles.border}`, textAlign: 'center' }}>
                <span style={{ 
    ...
    <Select
        ...
        styles={{
            ...
            option: (provided, state) => ({
                ...
                '&:hover': {
                    backgroundColor: state.isSelected ? '#fd7e14' : (theme === 'dark' ? '#495057' : '#f8f9fa')
                }
            }),
        ...
        placeholder={loadingFiles ? "Loading files..." : "Search and select file..."}
        isClearable
        isSearchable
