...
function DynamicForm() {
    ...
    // Updated fetchEditData to handle multiple entries properly
        ...
        try {
            ...
            if (response.data && response.data.status === 'success' && response.data.data && response.data.data.length > 0) {
                ...
                formFieldNames.forEach(formFieldName => {
                    // Check for all possible entry variations
    ...
    const handleSubmit = async (e) => {
        ...
        if (isEditMode && editId) {
            // Group data by categories and entries for update
            const groupedData = {};
            ...
            // Group data by categories and entries for insert
            const groupedData = {};
