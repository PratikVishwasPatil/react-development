...
const ChallanDetailPage = ({ vendorId, fileId, dcId, onBack }) => {
    ...
    useEffect(() => {
        // Fetch challan details from API
        const fetchChallanDetails = async () => {
            ...
            try {
                const response = await fetch(
                    `https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/Store/getAsslyDeliveryChallanFullApi.php?vendor=${vendorId}&file=${fileId}&dc_id=${dcId}`
                );
                ...
                if (data.status && data.header) {
                    // Update header data
                    setChallanData({
                        dcNumber: data.header.dc_no || '',
                        date: data.header.dc_date || '',
                        gstin: data.header.gst_no?.trim() || '',
                        fileName: data.header.file_name || '',
                        customerName: data.header.vendor_name || '',
                        vendorName: data.header.vendor_name || '',
                        address: data.header.address || ''
                    });
                    
                    // Update items data
                    // In the useEffect where you fetch data, update the items mapping:
if (data.items && data.items.length > 0) {
    const formattedItems = data.items.map((item, index) => ({
        ...
        unit: 'NOS',
        kg: item.kg || '',
        approxValue: item.basic_value || item.approx_value || '',  // FIX: Changed to basic_value
        tax: item.gst || '18',
        cgst: item.cgst || 0,
        sgst: item.sgst || 0
    }));
    setChallanItems(formattedItems);
}
                    ...
                    if (data.totals && data.summary) {
                        setApiTotals({
                            ...
                            basicValue: data.summary.total_basic_value || 0,
                            taxValue: data.summary.tax_value || 0,
                            grandValue: data.summary.grand_value || 0
                        });
                    }
                }
            } catch (error) {
                console.error('Error fetching challan details:', error);
                alert('Error loading challan details: ' + error.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const removeRow = (id) => {
        if (challanItems.length > 1) {
            const filtered = challanItems.filter(item => item.id !== id);
            const renumbered = filtered.map((item, index) => ({
                ...item,
                srNo: index + 1
            }));
            setChallanItems(renumbered);
        }
    };
    ...
    const calculateTotals = () => {
        ...
        const totalQty = challanItems.reduce((sum, item) => sum + (parseFloat(item.quantity) || 0), 0);
        const totalKg = challanItems.reduce((sum, item) => sum + (parseFloat(item.kg) || 0), 0);
        const basicValue = challanItems.reduce((sum, item) => sum + (parseFloat(item.approxValue) || 0), 0);
        const taxValue = challanItems.reduce((sum, item) => {
            const value = parseFloat(item.approxValue) || 0;
            const taxRate = parseFloat(item.tax) || 0;
            return sum + (value * taxRate / 100);
        }, 0);
        const grandValue = basicValue + taxValue;

        return { totalQty, totalKg, totalTax: taxValue, basicValue, taxValue, grandValue };
    };
    ...
    const handleSubmit = async () => {
        ...
        if (challanItems.some(item => !item.description || !item.quantity)) {
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600', color: '#1f2937' }}>Loading challan details...</p>
                </div>
            </div>
        );
    }
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                value={challanData.gstin}
                                onChange={(e) => setChallanData({...challanData, gstin: e.target.value})}
                                placeholder="27AAECS4252M1Z1"
                                style={inputStyle}
                            />
                        </div>
                    </div>
                    ...
                    <div style={{ marginTop: '15px' }}>
                        <label style={labelStyle}>File: {challanData.fileName}</label>
                        <p style={{ 
                            margin: '5px 0 0 0', 
                            fontSize: '1rem', 
                            fontWeight: '600',
                            ...
                            To: {challanData.customerName || challanData.vendorName}
                        </p>
                        {challanData.address && (
                        ...
                        <table style={{
                            ...
                            <thead>
                                <tr style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)' }}>
                                    ...
                                    <th style={headerStyle}>Description of Material</th>
                                    <th style={headerStyle}>Assembly Name</th>
                                    ...
                                    <tr key={item.id} style={{
                                        ...
                                        <td style={cellStyle}>
                                            <input
                                                ...
                                                value={item.srNo}
                                                readOnly
                                                style={{...tableInputStyle, textAlign: 'center', fontWeight: '600'}}
                                            />
                                        </td>
...
<td style={cellStyle}>
    <input
        ...
        readOnly
        style={{...tableInputStyle, backgroundColor: '#f3f4f6', cursor: 'not-allowed'}}
    />
</td>
<td style={cellStyle}>
    <input
...
<td style={cellStyle}>
    <input
        type="number"
        value={item.approxValue}  // FIX: Changed from basicValue to approxValue
        readOnly
        style={{...tableInputStyle, backgroundColor: '#f3f4f6', cursor: 'not-allowed'}}
    />
</td>
<td style={cellStyle}>
    ...
    />
                                ...
                                <tr style={{ backgroundColor: '#f3f4f6', fontWeight: '700' }}>
                                    ...
                                    <td style={{...cellStyle, textAlign: 'center', color: '#2563eb', fontSize: '1.1rem'}}>
                                        {parseFloat(totals.totalKg).toFixed(2)}
                                    </td>
                    ...
                    <div style={{
                        ...
                        <div style={summaryBoxStyle}>
                            <span style={{ fontSize: '0.9rem', color: '#6b7280' }}>TOTAL BASIC VALUE</span>
                            <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#1f2937' }}>
                                ₹ {parseFloat(totals.basicValue).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                            </span>
                        </div>
                        <div style={summaryBoxStyle}>
                        ...
                        <div style={summaryBoxStyle}>
                            <span style={{ fontSize: '0.9rem', color: '#6b7280' }}>GRAND VALUE</span>
                            <span style={{ fontSize: '1.5rem', fontWeight: '700', color: '#059669' }}>
                                ₹ {parseFloat(totals.grandValue).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                            </span>
                        </div>
                    </div>
...
const SecondChallanGrid = () => {
    ...
    const fetchChallanData = async (fy = financialYear) => {
        ...
        try {
            const response = await fetch(
                `https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/Store/DcAssemblyListApi.php?financial_year=${fy}`
            );
    ...
    const downloadExcel = () => {
        ...
        try {
            const params = {
                fileName: `SecondChallan_${financialYear}_${new Date().toISOString().split('T')[0]}.csv`,
                allColumns: true,
                onlySelected: false
            };
    ...
    if (showDetailPage && selectedChallan) {
        return (
            <ChallanDetailPage
                vendorId={selectedChallan.vendorId}
                fileId={selectedChallan.fileId}
                dcId={selectedChallan.dcId}
                onBack={() => {
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600' }}>Loading delivery challans...</p>
                    <style>{`
                        @keyframes spin {
                            to { transform: rotate(360deg); }
                        }
                    ...
                    <div style={{
                        ...
                        {rowData.length === 0 ? (
                            <div style={{
                                ...
                                <h5 style={{ marginBottom: '10px', fontSize: '1.25rem' }}>No challan data available</h5>
                            ...
                            >
                                <AgGridReact
                                    ...
                                    onGridReady={(params) => {
                                        console.log('Delivery Challan Grid ready');
