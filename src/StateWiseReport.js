...
const StateWiseReport = () => {
    ...
    const generateColumnDefs = (headers) => {
        ...
        // Dynamic columns based on header data
        // Headers: ["State", "Surya-Supply", "SM-Files", "Surya Labour", "Galaxy Labour", "AMC", "Total"]
        const dynamicColumns = headers.slice(1).map((header, index) => {
            const isTotal = header.toLowerCase() === 'total';
            const colors = ['#059669', '#0891b2', '#7c3aed', '#dc2626', '#ea580c', '#16a34a'];
    ...
    const fetchStateWiseData = async (fy = financialYear) => {
        if (!fy) {
        ...
        setLoading(true);
        try {
            const url = `https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/summary_report/stateWiseReportApi.php?financial_year=${fy}`;
            ...
            if (data.status && Array.isArray(data.dataArr)) {
                ...
                showToast(`Loaded ${data.dataArr.length} state records for FY ${fy}`, 'success');
            } else {
                throw new Error("Failed to fetch state-wise report data");
            }
        } catch (error) {
            console.error("Error fetching state-wise report data:", error);
    ...
    const handleSubmit = () => {
        ...
        fetchStateWiseData(financialYear);
    };
    ...
    const downloadExcel = () => {
        ...
        try {
            const params = {
                fileName: `StateWiseReport_${financialYear}_${new Date().toISOString().split('T')[0]}.csv`,
                allColumns: true,
                onlySelected: false
            };
    ...
    const calculateTotals = () => {
        ...
        // Initialize totals for each column (using header names, excluding "State")
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600' }}>Loading state-wise report data...</p>
                    <style>{`
                        @keyframes spin {
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <small style={{ opacity: 0.8, display: 'block', marginTop: '0.5rem', fontSize: '0.95rem' }}>
                                    {rowData.length} states | FY {financialYear}
                    ...
                    <div style={{
                        ...
                        {rowData.length === 0 ? (
                            <div style={{
                                ...
                                <h5 style={{ marginBottom: '10px', fontSize: '1.25rem' }}>No state-wise data available</h5>
                            ...
                            >
                                <AgGridReact
                                    ...
                                    onGridReady={(params) => {
                                        console.log('State-wise Report Grid ready');
