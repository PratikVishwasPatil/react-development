...
const SheetMetalDcGrid = () => {
    ...
    const fetchDcData = async (fy = financialYear) => {
        ...
        try {
            const response = await fetch(
                `http://erp.suryaequipments.com/Surya_React/surya_dynamic_api/Store/SmetalDcListApi.php?financial_year=${fy}`
            );
    ...
    const downloadExcel = () => {
        ...
        try {
            const params = {
                fileName: `SheetMetalDC_${financialYear}_${new Date().toISOString().split('T')[0]}.csv`,
                allColumns: true,
                onlySelected: false
            };
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600' }}>Loading Sheet Metal DC records...</p>
                    <style>{`
                        @keyframes spin {
                        ...
                        <div style={{
                            ...
                            <div>
                                <h4 style={{ margin: 0, fontSize: '1.75rem', fontWeight: '800' }}>
                                    📋 Sheet Metal DC Vendor Dashboard
                                </h4>
                    ...
                    <div style={{
                        ...
                        {rowData.length === 0 ? (
                            <div style={{
                                ...
                                <h5 style={{ marginBottom: '10px', fontSize: '1.25rem' }}>No Sheet Metal DC data available</h5>
                            ...
                            >
                                <AgGridReact
                                    ...
                                    onGridReady={(params) => {
                                        console.log('Sheet Metal DC Grid ready');
...
// Detail Component (placeholder - will be in next artifact)
const SheetMetalDcDetail = ({ vendorId, fileId, dcId, sequenceCount, onBack }) => {
    const [loading, setLoading] = useState(false);
    ...
    const handleSubmit = async () => {
        ...
        console.log('Submitting Sheet Metal DC data:', submissionData);
        alert('Sheet Metal DC data submitted successfully!');
    };
    ...
    useEffect(() => {
        const fetchDcDetails = async () => {
            ...
            try {
                const response = await fetch(
                    `http://erp.suryaequipments.com/Surya_React/surya_dynamic_api/Store/SmetalDcDetailsApi.php?vednor=${vendorId}&file=${fileId}&dc_id=${dcId}&sequenceCount=${sequenceCount}`
                );
                ...
                if (result.status === 'success' && result.data) {
                    ...
                    if (data.materials && data.materials.length > 0) {
                        const formattedMaterials = data.materials.map((item, index) => ({
                            ...
                            widthRfd: item.width_rfd || '',
                            lengthRfd: item.length_rfd || '',
                            rfdQty: item.rfd_qty || '',
                ...
                <div style={{
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '700' }}>
                            Delivery Challan for Sheet Metal Job Work
                        </h3>
                        ...
                        <table style={{
                            ...
                            <thead>
                                <tr style={{ background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' }}>
                                    ...
                                    <th style={headerStyle}>Width (RFD)</th>
                                    <th style={headerStyle}>Length (RFD)</th>
                                    ...
                                    <th style={headerStyle}>Weight Source</th>
                                </tr>
                            </thead>
                            <tbody>
                                {materials.map((item, index) => (
                                    <tr key={item.id} style={{
                                        ...
                                        <td style={cellStyle}>
                                            <div style={{
                                                ...tableInputStyle, 
                                                backgroundColor: item.weightSource === 'grn' ? '#dcfce7' : '#fef3c7', 
                                                textAlign: 'center',
...
export default SheetMetalDcGrid;
