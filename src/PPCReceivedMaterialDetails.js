...
const PPCProjectListGrid = () => {
    ...
    const fetchFileName = async () => {
        try {
            const response = await fetch("https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/GetFilenameApi.php", {
                method: "POST",
                headers: {
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                setFileName(data.data[0].FILE_NAME || data.data[0].file_name || 'Unknown File');
            } else {
    ...
    const fetchPPCProjectDataDetails = async () => {
        ...
        try {
            const response = await fetch(
                "https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/PPC/get_ReceviedMaterialList_23.php",
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (data.status === "success" && Array.isArray(data.data)) {
                ...
                setRowData(data);
                showToast(`Loaded ${data.length} records for File ${fileid}`, 'success');
            } else {
                setRowData([]);
    ...
    const downloadExcel = () => {
        ...
        try {
            const params = {
                fileName: `ReceivedMaterialDetails_${fileName || 'Data'}_${new Date().toISOString().split('T')[0]}.csv`,
                allColumns: true,
                onlySelected: false
            };
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <label style={{ fontSize: '0.875rem', whiteSpace: 'nowrap' }}>File:</label>
                                <span style={{
                                    fontWeight: 'bold',
                                    fontSize: '0.9rem',
