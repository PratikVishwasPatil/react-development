...
const AssignRfdMaterialManager = () => {
    ...
    const [selectedToRows, setSelectedToRows] = useState([]);
    ...
    const [selectedFromRows, setSelectedFromRows] = useState([]);
    ...
    const handleToCheckboxChange = (index) => {
        setSelectedToRows(prev => {
    ...
    const handleFromCheckboxChange = (index) => {
        setSelectedFromRows(prev => {
            if (prev.includes(index)) {
                return prev.filter(i => i !== index);
            } else {
                return [...prev, index];
    ...
    const handleAssignData = async () => {
        ...
        if (selectedFromRows.length === 0) {
            alert("Please select at least one material from 'Assign From' section");
            return;
        }

        if (selectedFromRows.length > 1) {
            alert("Please select only ONE material from 'Assign From' section");
            return;
        }

        if (selectedToRows.length === 0) {
            alert("Please select at least one material from 'Assign To' section");
            return;
        }

        const fromIndex = selectedFromRows[0];
        const fromRow = assignFromData[fromIndex];
        const fromQty = assignFromQuantities[fromIndex];

        if (!fromQty || parseFloat(fromQty) <= 0) {
            alert("Please enter a valid quantity for the selected 'Assign From' material");
            return;
        }

        // Validate against available stock
        if (parseFloat(fromQty) > parseFloat(fromRow.stock)) {
            alert(`Insufficient stock! Available: ${fromRow.stock}, Requested: ${fromQty}`);
            return;
        }

        // Check if all selected "To" rows have quantities
        for (let toIndex of selectedToRows) {
            const toQty = assignToQuantities[toIndex];
            if (!toQty || parseFloat(toQty) <= 0) {
                alert("Please enter valid quantities for all selected 'Assign To' materials");
                return;
            }
        }
        ...
        try {
            ...
            // Prepare data for API based on active tab
            const toDataString = selectedToRows.map(index => {
                const row = assignToData[index];
                const qty = assignToQuantities[index];
                return `${index}#${row.tableId}~${qty}`;
            }).join(',');

            // Format for store1: 0#fromRowId~weight~height~color~quantity
            let fromDataString;
            
            if (activeTab === 'Sheet Metal') {
                fromDataString = `0#${fromRow.id}~${fromRow.weight || '0'}~${fromRow.height || '0'}~${fromRow.colour || 'N/A'}~${fromQty}`;
            } else if (activeTab === 'Fabrication') {
                fromDataString = `0#${fromRow.id}~${fromRow.inmm || '0'}~0~${fromRow.color || 'N/A'}~${fromQty}`;
            } else if (activeTab === 'Foundation') {
                fromDataString = `0#${fromRow.id}~${fromRow.size || '0'}~${fromRow.length || '0'}~${fromRow.color || 'N/A'}~${fromQty}`;
            }
            ...
            if (activeTab === 'Sheet Metal') {
                apiEndpoint = 'saveSmetalAssignRfdData.php';
                materialType = 'smetal';
            } else if (activeTab === 'Fabrication') {
                apiEndpoint = 'saveFabAssignRfdData.php';
                materialType = 'fab';
            } else if (activeTab === 'Foundation') {
                apiEndpoint = 'saveFoundAssignRfdData.php';
                materialType = 'foundation';
            }

            const requestData = {
                to: selectedAssignToFile,
                store: toDataString,
                from: selectedAssignFromFile,
                store1: fromDataString,
                type: materialType,
                activeTab: activeTab
            };
            ...
            if (result.success) {
                alert(result.message || "Data assigned successfully!");
                ...
                alert("Error: " + (result.message || "Failed to assign data"));
            }

        } catch (error) {
            console.error("Error assigning data:", error);
            alert("An error occurred while assigning data. Please try again.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const renderAssignToTable = () => {
        ...
        if (activeTab === 'Sheet Metal') {
            ...
            columns = [
                { key: 'checkbox', label: '#', width: '60px' },
                { key: 'material_name', label: 'RFD Material Name' },
                ...
                <table style={{
                    ...
                    <tbody>
                        {assignToData.map((row, index) => (
                            <tr key={index} style={{
                                backgroundColor: selectedToRows.includes(index) 
                                    ? (theme === 'dark' ? '#334155' : '#dbeafe')
                                    : (index % 2 === 0 ? themeStyles.tableRowEven : themeStyles.tableRowOdd)
                            }}>
                                {columns.map((col, colIdx) => {
                                    if (col.key === 'checkbox') {
                                        return (
                                            <td key={colIdx} style={{...bodyCellStyle, textAlign: 'right'}}>
                                                <input 
                                                    type="checkbox" 
                                                    checked={selectedToRows.includes(index)}
                                                    onChange={() => handleToCheckboxChange(index)}
                                                    style={{ 
                                                        cursor: 'pointer',
                                                        width: '18px',
                                                        height: '18px',
                                        ...
                                        return (
                                            <td key={colIdx} style={{...bodyCellStyle, textAlign: 'right'}}>
                                                <input
                                                    ...
                                                    disabled={!selectedToRows.includes(index)}
                                                    style={{
                                                        width: '90px',
                                                        padding: '6px 10px',
                                                        border: `2px solid ${themeStyles.inputBorder}`,
                                                        borderRadius: '6px',
                                                        backgroundColor: selectedToRows.includes(index) ? themeStyles.inputBg : '#e2e8f0',
                                                        color: themeStyles.inputColor,
                                                        fontSize: '13px',
                                                        fontWeight: '500',
                                                        transition: 'all 0.2s ease',
                                                        cursor: selectedToRows.includes(index) ? 'text' : 'not-allowed',
                                                        textAlign: 'right'
                                                    }}
                                                    onFocus={(e) => {
                                                        if (selectedToRows.includes(index)) {
    ...
    const renderAssignFromTable = () => {
        ...
        if (activeTab === 'Sheet Metal') {
            columns = [
                { key: 'checkbox', label: '#', width: '60px' },
                { key: 'rfd_name', label: 'Material Name' },
                ...
                <table style={{
                    ...
                    <tbody>
                        {assignFromData.map((row, index) => (
                            <tr key={index} style={{
                                backgroundColor: selectedFromRows.includes(index)
                                    ? (theme === 'dark' ? '#334155' : '#dbeafe')
                                    : (index % 2 === 0 ? themeStyles.tableRowEven : themeStyles.tableRowOdd)
                            }}>
                                {columns.map((col, colIdx) => {
                                    if (col.key === 'checkbox') {
                                        return (
                                            <td key={colIdx} style={{...bodyCellStyle, textAlign: 'right'}}>
                                                <input 
                                                    type="checkbox" 
                                                    checked={selectedFromRows.includes(index)}
                                                    onChange={() => handleFromCheckboxChange(index)}
                                                    style={{ 
                                                        cursor: 'pointer',
                                                        width: '18px',
                                                        height: '18px',
                                        ...
                                        return (
                                            <td key={colIdx} style={{...bodyCellStyle, textAlign: 'right'}}>
                                                <input
                                                    ...
                                                    disabled={!selectedFromRows.includes(index)}
                                                    style={{
                                                        width: '100px',
                                                        padding: '6px 10px',
                                                        border: `2px solid ${themeStyles.inputBorder}`,
                                                        borderRadius: '6px',
                                                        backgroundColor: selectedFromRows.includes(index) ? themeStyles.inputBg : '#e2e8f0',
                                                        color: themeStyles.inputColor,
                                                        fontSize: '13px',
                                                        fontWeight: '500',
                                                        transition: 'all 0.2s ease',
                                                        cursor: selectedFromRows.includes(index) ? 'text' : 'not-allowed',
                                                        textAlign: 'right'
                                                    }}
                                                    onFocus={(e) => {
                                                        if (selectedFromRows.includes(index)) {
                                                            e.target.style.borderColor = '#3b82f6';
                                                            e.target.style.boxShadow = '0 0 0 3px rgba(59, 130, 246, 0.1)';
                                                        }
                                                    }}
                                    ...
                                    if (col.key === 'stock') {
                                        return (
                                            <td key={colIdx} style={{...bodyCellStyle, textAlign: 'right', fontWeight: '600', color: '#3b82f6'}}>
                                                {row[col.key] || '0'}
                                            </td>
                                        );
                                    }
                                    
                                    const cellValue = row[col.key] || (col.key === 'colour' || col.key === 'storelocation' || col.key === 'color' || col.key === 'location' ? 'N/A' : '0');
                                    return (
                                        <td key={colIdx} style={{
                                            ...bodyCellStyle,
                                            textAlign: 'right',
                ...
                <div style={{
                        ...
                        <div style={{ marginBottom: '24px' }}>
                            ...
                            <Select
                                ...
                                options={assignToFiles.map(file => ({
                                    value: file.FILE_ID,
                                    label: `${file.FILE_ID} - ${file.FILE_NAME}`
                                }))}
                                styles={{
                    ...
                    <div style={{ 
                        ...
                        <button
                            ...
                            style={{
                                padding: '14px 40px',
                                background: submitting ? '#94a3b8' : themeStyles.buttonGradient,
                                color: '#ffffff',
                                border: 'none',
                                borderRadius: '10px',
                                fontSize: '15px',
