...
const MaterialReplaceList = () => {
    ...
    // API URLs - Updated to MaterialReplaceListApi
    const STOCK_API_URL = "https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/PPC/MaterialReplaceListApi.php";
    ...
    // Fetch stock data - Updated for new API structure
    const fetchStockData = async (financialYear) => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.status === "success" && data.data) {
                // Map the API fields to match component's expected field names
                const mappedData = data.data.map(item => ({
                    ...
                    lastDispatchDate: item.lastDispatchDate
                }));
    ...
    const onSelectionChanged = (event) => {
        ...
        if (autoNavigateOnSelect && selectedData.length === 1) {
            ...
            setTimeout(() => {
                // Navigate to the material replace detail page with FILE_ID
                navigate(`/ppc/material-replace/${selectedRecord.FILE_ID}`);
            }, 500);
        }
    };
    ...
    // AG Grid column definitions - Updated with statusColor support
            ...
            cellStyle: params => {
                if (params.data && params.data.statusColor) {
                    return { 
                        ...
                        color: getTextColor(params.data.statusColor),
                        fontWeight: 'bold',
                        padding: '8px'
                    };
                }
    ...
    const getRowStyle = params => {
        if (params.data && params.data.statusColor) {
            return { 
                borderLeft: `4px solid ${params.data.statusColor}`
            };
        }
        return null;
    };
    ...
    const onGridReady = (params) => {
        console.log('Material Replace List Grid is ready');
    ...
    const handleExportCSV = () => {
        if (gridRef.current && gridRef.current.api) {
            gridRef.current.api.exportDataAsCsv({
                fileName: `Material_Replace_List_${selectedYear}_${new Date().toISOString().split('T')[0]}.csv`
            });
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p className="mt-3">Loading material replace data...</p>
                </div>
            </div>
        );
    }
                    ...
                    <Card.Header style={{
                        ...
                        <Row className="align-items-center">
                            <Col xs={12} lg={6} className="mb-2 mb-lg-0">
                                <h4 className="mb-0">Material Replace List</h4>
                    ...
                    <Card.Body style={{
                        ...
                        {stockData.length === 0 ? (
                            <div style={{
                                ...
                                <h5>No material replace data available</h5>
