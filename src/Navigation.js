    ...
    const fetchMenuItems = async () => {
      try {
       ...
       const userId = sessionStorage.getItem('userId');
        // alert(userId);
        // const userId =1;
        const response = await axios.get(`https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/dynamic_navbar.php?user_id=${userId}`, {
  ...
  return (
    <>
      ...
      >
        <Container fluid>
          {/* Brand with Logo */}
          <Link to="/dashboard" className="navbar-brand-logo" onClick={closeAllDropdowns}>
            <img 
              src={`${process.env.PUBLIC_URL}/surya-logo.png`} 
              alt="Surya Logo" 
