...
const PPCAssemblyMaterialSwapDetails = () => {
  const fileIdObj = useParams();
  const fileId = fileIdObj.fiileId;
  ...
  const fetchMaterialSwapData = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/PPC/AssemblySwapMaterialDetailsApi.php?file_id=${fileId}`
      );
      ...
      if (result.status === "success" && Array.isArray(result.data)) {
        const mappedData = result.data.map((item, index) => ({
          ...
          assignedBy: item.swap_by || "",
          timestamp: item.swap_date || "",
  ...
  // FIXED: Editable input cell component with proper value tracking
  const EditableInputCell = (props) => {
    ...
    const handleChange = (e) => {
      ...
      // Force refresh to ensure grid reflects changes
      api.refreshCells({ 
        rowNodes: [node], 
  ...
  const saveButtonRenderer = (params) => {
    const handleSave = async () => {
      try {
        // Get the latest data from the node
        const currentData = params.node.data;
        
        console.log("Saving data for:", currentData);
        ...
        formData.append("rowid", currentData.rowId);
        formData.append("materialName", currentData.materialDescription);
        formData.append("unit", currentData.unit);
        formData.append("qty", currentData.qty || "0");
        formData.append("comment", currentData.comment || "");
        formData.append("revision", revision);

        if (currentData.materialDescription === "Total") {
          formData.append("totalId", currentData.rowId);
          formData.append("total", currentData.total || "0");
        }

        console.log("Sending data to API:", Object.fromEntries(formData));

        const response = await fetch(
          `${API_BASE_URL}/PPC/UpdateAssemblyMaterialApi.php`,
          {
            method: "POST",
            body: formData,
          }
        ...
        const button = document.getElementById(`save-btn-${currentData.srNo}`);
  ...
  // FIXED: Add material with proper grid refresh
  const handleAddMaterial = async () => {
    ...
    try {
      ...
      const checkResponse = await fetch(
        `${API_BASE_URL}/checkSwapMaterialApi.php`,
        {
          method: "POST",
          body: checkFormData,
        }
      ...
      addFormData.append("comment", widthField ? ` ${widthField}` : "");
      ...
      const addResponse = await fetch(
        `${API_BASE_URL}/PPC/AddAssemblyMaterialApi.php`,
        {
          method: "POST",
          body: addFormData,
        }
      ...
      if (addResult.status === "success") {
        ...
        // Force grid to refresh after a short delay to ensure data is loaded
        setTimeout(() => {
  ...
  const selectStyles = {
    control: (base, state) => ({
      ...
      boxShadow: state.isFocused
        ? `0 0 0 3px ${themeStyles.inputFocus}20`
        : "none",
      "&:hover": {
      ...
      <div className={`container-fluid ${isFullScreen ? "p-0" : ""}`}>
        <div
          ...
          style={{
            ...
            color: themeStyles.color,
            border:
              theme === "dark" ? "1px solid #2d3748" : "1px solid #e2e8f0",
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        value={quantityField}
                        onChange={(e) => handleNumberInput(e, setQuantityField)}
...
export default PPCAssemblyMaterialSwapDetails;