...
function DynamicForm() {
    ...
    const [selectedFileType, setSelectedFileType] = useState(''); // Track selected file type
                ...
                fetchedCategories.forEach(category => {
                    if (category.id == 11) {
                        category.fields = category.fields
                            .filter(field => field.field_name.toLowerCase().includes('file type'))
                            .map(field => {
    ...
    const removeTableRow = (groupName, rowIndex) => {
        setTableRows(prev => {
            ...
            setFormData(prevFormData => {
                ...
                const groupFields = getFieldsByGroup(groupName);
                groupFields.forEach(field => {
                    delete newFormData[`${field.field_name}_${rowIndex}`];
                });
                return newFormData;
            });
    ...
    const handleChange = (e, change_rule_id, entryIndex = 0) => {
        ...
        // Track file type selection
        if (name.toLowerCase().includes('file type')) {
            setSelectedFileType(value.toLowerCase());
        }

        const fieldNameLower = name.toLowerCase();
        if (fieldNameLower.includes('concerned') && fieldNameLower.includes('labour')) {
                    ...
                    if (Array.isArray(allRuleResults)) {
                        ...
                        allRuleResults.forEach(ruleResult => {
                            ...
                            if (formulla) {
                                ...
                                // Dynamic field addition based on group
                                const updatedCategories = futureCategories.map(category => {
                                    if (category.id == '11') {
                                        const filteredFields = category.fields
                                            .filter(field => {
                                                return field.field_name.toLowerCase().includes('file type') ||
                                                    (field.group && field.group.toLowerCase() === value.toLowerCase());
                                            })
                                            .map(field => {
                                ...
                                setCategories(updatedCategories);
                                if (showHideFormulla && highlighted_fields) {
                                    const groupFields = getFieldsByGroup(value.toLowerCase());
                                    if (groupFields.length > 0) {
                                        setTableRows(prev => ({
                                            ...prev,
                                            [value.toLowerCase()]: prev[value.toLowerCase()] || [0]
                                        }));

                                        // Initialize form data for first row
                                        setFormData(prevFormData => {
                                            const newFormData = { ...prevFormData };
                                            groupFields.forEach(field => {
                                                if (!newFormData.hasOwnProperty(`${field.field_name}_0`)) {
                                                    newFormData[`${field.field_name}_0`] = field.field_type === 'checkbox' || field.field_type === 'multiselect'
                                                        ? []
                                                        : '';
                                                }
                                            });
    ...
    const renderTable = (groupName) => {
        ...
        const hasTableFields = groupFields.some(field => !isFieldHighlighted(field.field_name));
        if (!hasTableFields) {
            return null;
        }
                                ...
                                <tr key={rowIndex}>
                                    ...
                                    {groupFields.map((field, fieldIdx) => (
                                        <td key={fieldIdx} style={{ minWidth: '150px' }}>
                                            {renderTableField(field, rowIndex)}
                                        </td>
                                    ))}
    ...
    const renderTableField = (field, entryIndex = 0) => {
    ...
    // Get grouped fields - only for category 11
    // const getGroupedFields = (categoryId) => {
    //     if (categoryId != 11) return null;

    //     const groups = {};
    //     futureCategories.forEach(category => {
    //         if (category.id == categoryId) {
    //             category.fields.forEach(field => {
    //                 if (field.group && field.group.toLowerCase() !== '' && !field.field_name.toLowerCase().includes('file type')) {
    ...
    //                 if (field.group && field.group.toLowerCase() !== '' && !field.field_name.toLowerCase().includes('file type')) {
    //                     const groupName = field.group.toLowerCase();
    //                     // Only add to table if not highlighted (for AMC) or if not AMC type
    //                     // if (selectedFileType.toLowerCase() === 'amc') {
    //                     // NEW - check if any fields should be shown as table vs form
    //                     if (groupFields.some(field => !isFieldHighlighted(field.field_name))) {
    //                         if (!isFieldHighlighted(field.field_name)) {
    //                             if (!groups[groupName]) {
    //                                 groups[groupName] = [];
    //                             }
    //                             groups[groupName].push(field);
            ...
            if (category.id == categoryId) {
                category.fields.forEach(field => {
                    if (field.group && field.group.toLowerCase() !== '' && !field.field_name.toLowerCase().includes('file type')) {
                        ...
                        // Show fields in table only if:
                        // 1. The selected file type matches the field's group AND
                        // 2. There are highlighted fields defined (meaning table should be used) AND
                        // 3. This field is NOT highlighted (non-highlighted fields go in table)
                        if (selectedFileType.toLowerCase() === groupName &&
                            highlightedFields.length > 0 &&
                            !isFieldHighlighted(field.field_name)) {
    ...
    const getHighlightedFieldsForForm = (categoryId, selectedFileType) => {
        // Only show highlighted fields as regular form fields when there are highlighted fields defined
        if (categoryId != 11 || !highlightedFields.length) return [];

        const highlightedFieldsForForm = [];
        futureCategories.forEach(category => {
            if (category.id == categoryId) {
                category.fields.forEach(field => {
                    if (field.group && field.group.toLowerCase() === selectedFileType.toLowerCase()) {
                        if (isFieldHighlighted(field.field_name)) {
                            highlightedFieldsForForm.push(field);
                        }
                    }
                });
            }
        ...
        return highlightedFieldsForForm;
    };



    // Get non-grouped fields for regular display
    const getNonGroupedFields = (categoryFields, selectedFileType) => {
        return categoryFields.filter(field => {
            // Always show file type field
            if (field.field_name.toLowerCase().includes('file type')) {
                return true;
            }

            // For fields with groups
            if (field.group && field.group.toLowerCase() !== '') {
                // If file type is selected and matches field group
                if (selectedFileType && field.group.toLowerCase() === selectedFileType.toLowerCase()) {
                    // If there are highlighted fields, don't show group fields here
                    // (they'll be handled by table or highlighted form fields)
                    if (highlightedFields.length > 0) {
                        return false;
                    }
                    // If no highlighted fields, show all group fields as regular fields
                    return true;
                }
                // If file type doesn't match or no file type selected, don't show
                return false;
            }

            // Show fields without group
            return true;
        });
    };
                                                ...
                                                >
                                                    ...
                                                    {(categoryEntries[category.category_key] || [0]).map((entryIndex, idx) => (
                                                        <div key={entryIndex} className="mb-4">
                                                            ...
                                                            <Row>
                                                                {/* Show non-grouped fields and conditional grouped fields */}
                                                                {getNonGroupedFields(category.fields, selectedFileType).map((field, fieldIndex) => (
                                                            ...
                                                            {/* Show grouped fields as regular fields for non-AMC types */}
                                                            {selectedFileType && (
                                                                <Row>
                                                                    {category.id == 11 && (() => {
                                                                        // Get current group fields for the selected file type
                                                                        const currentGroupFields = getFieldsByGroup(selectedFileType.toLowerCase());

                                                                        // If there are highlighted fields defined for this group, 
                                                                        // show only highlighted fields as regular form fields
                                                                        if (currentGroupFields && currentGroupFields.length > 0 && highlightedFields.length > 0) {
                                                                            // Some fields will be in table (non-highlighted), some in form (highlighted)
                                                                            return getHighlightedFieldsForForm(category.id, selectedFileType).map((field, fieldIndex) => (
                                                                                <Col md={4} key={field.field_name}>
                                                                                    {renderField(field, entryIndex)}
                                                                                </Col>
                                                                            ));
                                                                        } else if (currentGroupFields && currentGroupFields.length > 0) {
                                                                            // No highlighted fields defined, show all group fields as regular form fields
                                                                            return currentGroupFields.map((field, fieldIndex) => (
                                                                                <Col md={4} key={field.field_name}>
                                                                                    {renderField(field, entryIndex)}
                                                                                </Col>
                                                                            ));
                                                    ...
                                                    {/* // NEW - dynamic check */}
                                                    {category.id == 11 && selectedFileType && highlightedFields.length > 0 && getGroupedFields(category.id) &&
                                                        Object.keys(getGroupedFields(category.id)).length > 0 &&
