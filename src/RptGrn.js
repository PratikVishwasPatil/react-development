...
const FPDataConsolidatedReport = () => {
    ...
    const fetchReportData = async (from = '', to = '') => {
        ...
        try {
          ...
          const url = `https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/Report/get_GRNReport_23.php?${params}`;
    ...
    const getPinnedBottomRow = () => {
        ...
        const totals = rowData.reduce((acc, row) => {
            acc.poqty += Number(row.poqty || 0);
            acc.grnqty += Number(row.grnqty || 0);
            acc.grnwt += Number(row.grnwt || 0);
            acc.grnamt += Number(row.grnamt || 0);
    
            acc.total_basic_amt += Number(row.total_basic_amt || 0);
            acc.other_charges += Number(row.other_charges || 0);
            acc.total_amt += Number(row.total_amt || 0);
            acc.gst_amt += Number(row.gst_amt || 0);
        ...
        return [{
            // show TOTAL label in left-most visible column
            filename: 'TOTAL',
            supplier: 'TOTAL',
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600' }}>Loading GRN Report...</p>
                    <style>{`
                        @keyframes spin {
