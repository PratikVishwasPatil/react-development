...
const FPDataConsolidatedReport = () => {
    ...
    const generateColumnDefs = () => {
        const baseColumns = [
            { headerName: "File Name", valueGetter: p => p.data?.file_name },

  { headerName: "Advance", valueGetter: p => Number(p.data?.Advance || 0) ,type: 'indianCurrency' },

  { headerName: "Total Wages", valueGetter: p => Number(p.data?.["Total Wages"] || 0) ,type: 'indianCurrency'},

  { headerName: "8 Hours Wages", valueGetter: p => Number(p.data?.Wages_8_hours || 0) ,type: 'indianCurrency'},

  { headerName: "OT Charges", valueGetter: p => Number(p.data?.OT_Charges || 0),type: 'indianCurrency' },

  { headerName: "Incentive", valueGetter: p => Number(p.data?.Incentive || 0) ,type: 'indianCurrency'},

  { headerName: "Person", valueGetter: p => Number(p.data?.person || 0),type: 'indianCurrency'},

  { headerName: "Travelling", valueGetter: p => Number(p.data?.Travelling || 0),type: 'indianCurrency' },

  { headerName: "Lodging", valueGetter: p => Number(p.data?.Lodging || 0) ,type: 'indianCurrency'},

  { headerName: "Daily Allowance", valueGetter: p => Number(p.data?.Daily_allowence || 0) ,type: 'indianCurrency'},

  { headerName: "Local Travel", valueGetter: p => Number(p.data?.Local_travel || 0) ,type: 'indianCurrency'},

  { headerName: "Purchase", valueGetter: p => Number(p.data?.Purchase || 0) ,type: 'indianCurrency'},

  { headerName: "Comment", valueGetter: p => p.data?.Comment },

  { headerName: "Other", valueGetter: p => p.data?.other,type: 'indianCurrency' },

  { headerName: "Total", valueGetter: p => Number(p.data?.total || 0), cellStyle:{fontWeight:'700'} ,type: 'indianCurrency'},

  { headerName: "Created By", valueGetter: p => p.data?.created_by },

  { headerName: "Created At", valueGetter: p => p.data?.created_at }
        ];
...
gridRef.current?.api?.showLoadingOverlay();
        try {
            ...
            const url = `https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/summary_report/get_expense_tracker_total_summaray_report.php`;
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem', fontWeight: '600' }}>Loading weekly expense-total-summary-report...</p>
                    <style>{`
                        @keyframes spin {
                        ...
                        <div style={{
                            ...
                            <div>
                                <h4 style={{ margin: 0, fontSize: '1.75rem', fontWeight: '800' }}>
                                    📋 weekly expense-total-summary-report
                                </h4>
...
>
  {summaryConfig.map(card => (
    <SummaryCard
      ...
      theme={theme}
      label={card.label}
      value={summaryTotals[card.key] || 0}
      color={card.color}
