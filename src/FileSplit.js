...
const GridExample = () => {
    ...
    // ⬇️ Fetch financial years using the new API
    useEffect(() => {
    ...
    const fetchData = async (year = selectedOption?.value) => {
        if (!year) return; // Don't fetch if no year is selected
        
        setLoading(true);
        try {
            const response = await fetch("https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/project/getfileSplitList_23.php", {
                method: "POST",
                headers: {
    ...
    }, [selectedOption, isMobile, isFullScreen]);
    ...
    const handleEditRow = () => {
        ...
        navigate(`/marketing/project/UploadFPData/${selectedRow.fileid}`);
    };
    ...
    const onSelectionChanged = async (event) => {
        ...
        // Example: call API whenever selection changes
        if (selectedRow.length > 0 && selectedRow[0].fileid) {
            navigate(`/project/FileSplitDetails/${selectedRow[0].fileid}`);
        }
    };
                        ...
                        <Row className="align-items-center g-2">
                            ...
                            <Col xs={12} lg={6}>
                                <div style={{ marginBottom: "10px" }}>
                                    <label style={{ marginRight: "8px", color: "black" }}>Financial Year:</label>
                                    <Select
