...
const emptyShippingAddress = () => ({
  ...
  state: '',
  city: '',
  cityId: '',   // ✅ starts empty — validated only on save
  gstin: '',
  pan: '',
  address: '',
});
...
function CountryStateCity({ countryVal, stateVal, cityVal, cityIdVal, onCountryChange, onStateChange, onCityChange }) {
  ...
  // ✅ FIX: After cities load, if cityVal already exists (editing mode),
  //         find the city's id and fire onCityChange so cityId is always populated
  useEffect(() => {
    ...
    axios.post(`${BASE_URL}/getCity.php`, new URLSearchParams({ state: stateVal }))
      .then(r => {
        ...
        // ✅ If we already have a cityVal but no cityId (loaded from DB), resolve the id now
        if (cityVal && !cityIdVal) {
          const match = opts.find(o => o.value === cityVal);
          if (match) {
            onCityChange(match.value, match.id);
          }
        }
      })
      .catch(() => setCities([]));
    ...
    <>
      ...
      <Col xs={12} sm={4}>
        <div className="surya-select-wrap mb-3">
          ...
          <Select
            options={cities}
            // ✅ FIX: match by city name (cityVal) — cityId is internal, not for matching
  ...
  return (
    <div className="surya-address-card mb-4">
      ...
      <Row className="g-3 mt-1">
        {/* ✅ FIX: Pass cityIdVal so CountryStateCity knows when id is already resolved */}
        <CountryStateCity
...
export default function EditCompany() {
      ...
      try {
        ...
        if (shipData.length > 0) {
          setShippingAddresses(shipData.map(addr => ({
            ...
            state:   addr.state   || '',
            city:    addr.city    || '',
            // ✅ FIX: store cityId from DB — CountryStateCity will resolve it if missing
            cityId:  addr.city_id || '',
            gstin:   addr.GSTN    || '',
            pan:     addr.PAN     || '',
  ...
  const handleSaveShipping = async () => {
    for (let i = 0; i < shippingAddresses.length; i++) {
      ...
      // ✅ FIX: validate city by cityId OR city name — either is acceptable
          ...
          <Accordion activeKey={activeKey} onSelect={k => setActiveKey(k)}>
            ...
            <Accordion.Item eventKey="1" className="surya-accordion-item">
              ...
              <Accordion.Body>
                ...
                {/* ✅ FIX: Add button correctly calls addShippingAddress */}
            ...
            <Accordion.Item eventKey="2" className="surya-accordion-item">
              ...
              <Accordion.Body>
                ...
                {/* ✅ FIX: Add button correctly calls addBillingAddress */}
