...
const ManufacturingDataYearlyGrid = () => {
    ...
    data && data.length > 0
      ? Object.keys(data[0]).map((key) => ({
          ...
          floatingFilter: true,
          resizable: true,
          pinned: key === "Mktg_Owner" || key === "file" ? "left" : undefined,
        }))
      : [];
    ...
    // Fetch Billwise Debitors Data list data
    const fetchManufacturingDataYearly = async () => {
      try {
        const res = await axios.get(
          "https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/account/get_billwiseDebitorData.php"
        );
    
        if (res.data) {
          setRowData(res.data);
    
          // FIX: generate dynamic columns
          const cols = generateColumnDefs(res.data);
          setColumnDefs(cols);
    
        //   if (res.data[0].created_at) {
        //     const timestamp = moment(res.data[0].created_at).format(
        //       "DD-MM-YYYY HH:mm:ss"
        //     );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://www.erp.suryaequipments.com/Surya_React/surya_dynamic_api/account/ajaxUploadBillwiseDebitors.php",
        ...
        return (
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: themeStyles.color }}>
                    ...
                    <p style={{ marginTop: '1rem' }}>Loading Billwise Debitors Data...</p>
                </div>
            </div>
        );
    }
        ...
        <div style={{
            ...
            <Accordion defaultActiveKey="0">
              <Accordion.Item eventKey="0">
                <Accordion.Header>Upload Yearly Billwise Debitors Dataa</Accordion.Header>
                <Accordion.Body>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
                            <div style={{ flex: isMobile ? '1 1 100%' : '1 1 auto' }}>
                                <h4 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <span>📋</span>Upload Billwise Debitors</h4>
                    ...
                    <div style={{
                        ...
                        {rowData.length === 0 && !loading ? (
                            <div style={{
                                ...
                                <h5>No Billwise Debitors Dataa available</h5>
                                   {/* Last uploaded time */}
      {lastUpdated && (
        <div style={{ marginBottom: "10px", fontWeight: "bold" }}>
          Recent Uploaded Date: {lastUpdated}
        </div>
      )}
                                <button
                                    onClick={handleRefresh}
